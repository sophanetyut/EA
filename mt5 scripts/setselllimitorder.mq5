...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=SetSellLimitOrder(Symbol(),MM,LEVEL,STOPLOSS,TAKEPROFIT,EXPIRED);
...
//SetSellLimitOrder(symbol, Money_Management, level, StopLoss, Takeprofit, Expired);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
//----
...
//----  
   ...
   double volume=LotCount(symbol,Money_Management,ORDER_TYPE_SELL,lprice);
   if(volume<=0)
     {
...
//---- Checking correctness of a trade request
   ...
   string word="";
   StringConcatenate(word,__FUNCTION__,"(): <<< Set SellLimit order by ",symbol,"! >>>");
   Print(word);
...
//---- open Sell position and check the result of trade request
   if(!OrderSend(request,result) || result.retcode!=TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Failed to set SellLimit order by ",symbol,"!!! >>>");
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< SellLimit order by ",symbol," is set! >>>");
      PlaySound("ok.wav");
     }
   else
     {
