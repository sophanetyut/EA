...
input int   LEVEL1=300;     // Trigger level of the first order in points from the current price
input int   LEVEL2=300;     // Trigger level of the second order in points from the trigger level of first order
...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=SetBuyStopLimitOrder(Symbol(),MM,LEVEL1,LEVEL2,STOPLOSS,TAKEPROFIT,EXPIRED);
...
//SetBuyStopOrder(symbol, Money_Management, Orderlevel, level1, level2, StopLoss, Takeprofit, Expired);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
//----
   ...
   if(!Expired) time=0;
//----  
   if(!StopCorrect(symbol,level1)||!StopCorrect(symbol,level2)) return(TRADE_RETCODE_ERROR);
   double dlevel1=level1*point;
   double dlevel2=level2*point;
   double lprice1=NormalizeDouble(price+dlevel1,digit);
   double lprice2=NormalizeDouble(lprice1-dlevel2,digit);
//----  
   double volume=LotCount(symbol,Money_Management,ORDER_TYPE_BUY,lprice2);
   if(volume<=0)
     {
...
//---- Checking correctness of a trade request
   ...
   string word="";
   StringConcatenate(word,__FUNCTION__,"(): <<< Set BuyStopLimit order by ",symbol,"! >>>");
   Print(word);
...
//---- open BUY position and check the result of trade request
   if(!OrderSend(request,result) || result.retcode!=TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Failed to set BuyStopLimit order by ",symbol,"!!! >>>");
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< BuyStopLimit order by ",symbol," is set! >>>");
      PlaySound("ok.wav");
     }
   else
     {
