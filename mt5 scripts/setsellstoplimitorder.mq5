...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=SetSellStopLimitOrder(Symbol(),MM,LEVEL1,LEVEL2,STOPLOSS,TAKEPROFIT,EXPIRED);
...
//SetSellStopOrder(symbol, Money_Management, Orderlevel, level1, level2, StopLoss, Takeprofit, Expired);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
//----
...
//----  
   double volume=LotCount(symbol,Money_Management,ORDER_TYPE_SELL,lprice2);
   if(volume<=0)
     {
...
//---- open SELL position and check the result of trade request
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< SetSellStopLimit order by ",symbol," is set! >>>");
      PlaySound("ok.wav");
     }
   else
     {
