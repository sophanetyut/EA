...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=InvertPosition(Symbol(),VOLUME,DEVIATION,STOPLOSS,TAKEPROFIT);
...
//InvertPosition(symbol, Volume, deviation, StopLoss, Takeprofit);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
...
//----
   ...
   if(lot>MaxLot/2.0)
     {
      string word="";
      StringConcatenate(word,__FUNCTION__,"(): <<< The script is reverses the position by ",
                        symbol," only with a maximum volume ",DoubleToString(MaxLot/2.0,2),"! >>>");
      Print(word);
...
//---- Initializing structure of the MqlTradeRequest to open position
   ...
   if(!LotCorrect(symbol,lot,PosType) || !lot)
     {
      string word="";
      StringConcatenate(word,__FUNCTION__,"(): <<< For overturn ",sType," position ny ",symbol," are not enough funds! >>>");
      Print(word);
      PlaySound("timeout.wav");
      return(TRADE_RETCODE_DONE);
     }
   ...
   if(StopLoss)
     {
      ...
      request.sl=NormalizeDouble(request.price-sign*dStopLoss,digit);
     }
   else request.sl=0.0;
   ...
   if(Takeprofit)
     {
      ...
      request.tp=NormalizeDouble(request.price+sign*dTakeprofit,digit);
     }
   else request.tp=0.0;
//----
...
//---- Overturn position and check the result of trade request
   if(!OrderSend(request,result) || result.retcode!=TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Failed to overturn",sType," position by ",symbol,"!!! >>>");
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< ",sType," position by ",symbol," is overturned! >>>");
      PlaySound("ok.wav");
     }
   else
     {
