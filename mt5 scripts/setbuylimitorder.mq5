...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=SetBuyLimitOrder(Symbol(),MM,LEVEL,STOPLOSS,TAKEPROFIT,EXPIRED);
...
//SetBuyLimitOrder(symbol, Money_Management, level, StopLoss, Takeprofit, Expired);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
//----
...
//---- Checking correctness of a trade request
   ...
   string word="";
   StringConcatenate(word,__FUNCTION__,"(): <<< Set BuyLimit order by ",symbol,"! >>>");
   Print(word);
...
//---- open BUY position and check the result of trade request
   if(!OrderSend(request,result) || result.retcode!=TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Failed to set BuyLimit order by ",symbol,"!!! >>>");
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< BuyLimit order by ",symbol," is set! >>>");
      PlaySound("ok.wav");
     }
   else
     {
