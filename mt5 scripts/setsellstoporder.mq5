...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=SetSellStopOrder(Symbol(),MM,LEVEL,STOPLOSS,TAKEPROFIT,EXPIRED);
...
//SetSellStopOrder(symbol, Money_Management, level, StopLoss, Takeprofit, Expired);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
//----
...
//---- Checking correctness of a trade request
   ...
   string word="";
   StringConcatenate(word,__FUNCTION__,"(): <<< Set SellStop order by ",symbol,"! >>>");
   Print(word);
...
//---- open Sell position and check the result of trade request
   if(!OrderSend(request,result) || result.retcode!=TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Failed to set SellStop order by ",symbol,"!!! >>>");
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< SellStop order by ",symbol," is set! >>>");
      PlaySound("ok.wav");
     }
   else
     {
