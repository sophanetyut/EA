...
//----
   for(uint count=0; count<=RTOTAL && !IsStopped(); count++)
     {
      uint result=SellPositionOpen(Symbol(),LossMM,DEVIATION,STOPLOSS,TAKEPROFIT);
...
//SellPositionOpen(symbol, Loss_Money_Management, deviation, StopLoss, Takeprofit);
//+ - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -+
  {
//----
   if(!StopLoss) return(TRADE_RETCODE_ERROR);

   double volume=SellLotCount(symbol,Loss_Money_Management,StopLoss);
   if(volume<=0)
     {
...
//---- Checking correctness of a trade request
   ...
   string word="";
   StringConcatenate(word,__FUNCTION__,"(): <<< Open Sell position at ",symbol,"! >>>");
   Print(word);
...
//---- open Sell position and check the result of trade request
   if(!OrderSend(request,result) || result.retcode!=TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Failed to open Sell position at ",symbol,"!!! >>>");
   ...
   if(result.retcode==TRADE_RETCODE_DONE)
     {
      Print(word,"<<< Sell position at ",symbol," is opened! >>>");
      PlaySound("ok.wav");
     }
   else
     {
...
//----
   int digit=int(SymbolInfoInteger(symbol,SYMBOL_DIGITS));
   double point=SymbolInfoDouble(symbol,SYMBOL_POINT);
   double price_open=SymbolInfoDouble(symbol,SYMBOL_BID);
