...
void OnStart() {
    ...
    double sl = NormalizeDouble(m_symbol.Ask() - AccountPercentStopPips(lots) * point, (int)digits);    
    double tp = NormalizeDouble(m_symbol.Ask() + (AccountPercentStopPips(lots) * RewardRatio) * point, (int)digits); 
    if(!m_trade.PositionOpen(Symbol(), ORDER_TYPE_BUY, lots, m_symbol.Ask(), sl, tp))
